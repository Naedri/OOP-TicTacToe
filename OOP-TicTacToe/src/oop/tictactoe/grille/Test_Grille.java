	...
	void testGrilleAdjacent() {
		...
		for (int i=0;i < grille.getLignes(); ++i) {
			for (int j = 0; j < grille.getColonnes(); ++j) {
				...
				System.out.println("Il n existe pas de cellule non vide adjacente pour la cellule "+ i+"ligne "+j+"colonne.");
			}
		}
		...
		for (int i=0;i < grille.getLignes(); ++i) {
			for (int j = 0; j < grille.getColonnes(); ++j) {
				if ((i==0 && j==1) || (i==1 && j==1) || (i==1 && j==0) ) {
					...
					System.out.println("Il existe une cellule non vide adjacente pour la cellule "+ i+"ligne "+j+"colonne.");
