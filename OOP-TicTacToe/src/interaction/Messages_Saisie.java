...
public class Messages_Saisie {
	 ...
	 * messageResultat victoire ou non (càd "Continuez la partie!" ou "Le Joueur X a
	 * gagné la partie") messageTour indique qui doit jouer messagePermutation quel
	 * cellule de la grille ont été permutées
	 */
	 ...
	 * cellule par le joueur (l entrée ) ne verifie pas que la cellule est pleine ou
	 * non
	 */
			...
			try {
				if (estValideSaisie(saisie)) {
					...
					if (cellule.length == 2) {
						...
						--cellule[0];
						--cellule[1];
						if (cellule[0] >= 0 && cellule[0] < grille.length && cellule[1] >= 0
								&& cellule[1] < grille[0].length) {
							// sortie de la boucle
							saisieCorrecte = true;
						} else
...
//	}
	 ...
	 * comme cellule doit etre appele à la suite de saisirCellule()
	 * 
	...
	public static String afficherMessageCellule(Joueur j, int[] celluleCoord) {
		...
		return "Le joueur " + j.getJeton().getSymbole() + " a choisi la cellule ligne [" + saisieLigne + "], colonne ["
				+ saisieColonne + "].\n";
	}

	/**
	 * messageResultat indiquant victoire ou non il faut qu elle soit appelee après
	 * un coup joue (càd "C est au joueur suivant" ou "Le Joueur X a gagné la
	 * partie")
	 * 
